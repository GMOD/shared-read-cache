...
| `sizeOf(value)`     | `() => 1`     | Weighs a settled value against the budget. The default makes the budget an entry count. See [memory.md](memory.md#sizeof-is-the-point), and [consumers.md](consumers.md) for what each package weighs.                  |
